...
public class ControllerImpl implements Controller {
    ...
    public String exploreState(String stateName) {
        ...
        List<Explorer> explorersRetired = explorers.stream()
                .filter(explorer -> explorer.getEnergy() == 0)
                .collect(Collectors.toList());

        int countRetired = explorersRetired.size();
        this.exploredStates++;
    ...
    public String finalResult() {
        ...
        output.append(String.format(ConstantMessages.FINAL_STATE_EXPLORED, this.exploredStates));
